...
const TagsSearch: FC<IProps> = props => {
  ...
  function renderPopupContent() {
    return <div className={styles.tagContent}>
      <div className={styles.chiosed}>
        <div className={styles.tagsWord}><Translation>Select up to 5 tags, currently selected</Translation>{searchTags && searchTags.length}:</div>
      ...
      <div className={styles.actionButton}>
        <Button.Group>
          <Button type="primary" onClick={handleSubmit}><Translation>Confirm</Translation></Button>
          <span className={styles.reset} onClick={handleReset}><Translation>Reset</Translation></span>
        </Button.Group>
      </div>
    </div>;
  }
  ...
  function renderPlaceholder() {
    const { tagNames } = props;
    if (selectVisible) {
      return `${i18n.t('Selected')}${searchTags && searchTags.length}${i18n.t('Tags')}`;
    }
    return `${i18n.t('Selected')}${tagNames && tagNames.length}${i18n.t('Tags')}`;
  }
