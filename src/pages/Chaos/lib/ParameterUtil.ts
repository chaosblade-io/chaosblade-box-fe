              ...
              if (required) {
                if (value == null || value === '') {
                  ...
                  if (type === FunctionParameterConstants.PARAMETER_COMPONENT_TYPE_SELECT_REMOTE ||
                    ...
                    arg.errorMessage = '必选项，请选择！';
                  } else {
                    arg.errorMessage = '必填项，请输入内容！';
                  }
                  continue;
                } else {
...
const opposite = function(...args: IArgs[]) {
  ...
  arg1.errorMessage = `必须与参数<${arg2.name}>的值相反！`;
  arg2.errorMessage = `必须与参数<${arg1.name}>的值相反！`;
...
const one_only = function(...args: IArgs[]) {
  ...
  for (const arg of args) {
    ...
    arg.errorMessage = `与参数${_.map(otherArgs, ({ name }: IArgs) => `<${name}>`).join('、')}${args.length}选1填写!`;
  }
  return { error: true, args };
};
