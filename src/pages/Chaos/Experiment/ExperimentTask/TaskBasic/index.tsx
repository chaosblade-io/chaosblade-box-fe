...
export default function TaskBasic(props: TaskBasicProps) {
  ...
  return <div className={styles.basicContent}>
    ...
    {cronExpression && <div className={styles.basicItem}>
      <div className={styles.label}>定时运行</div>
    ...
    <div className={styles.basicItem}>
      <div className={styles.label}>演练时长</div>
      <div className={styles.value}>{renderDuration()}</div>
    </div>
    <div className={styles.basicItem}>
      <div className={styles.label}>演练进度</div>
      <div className={styles.value}>
        <Progress percent={progressBarWidth * 100}/>
      </div>
    </div>
    <div className={styles.basicItem}>
      <div className={styles.label}>演练结果</div>
      <div className={styles.value}>
        <TagGroup>
          <Tag type="normal" size="small">
            <span>运行成功：<span style={{ color: '#1E8E3E' }}>{stateCount.success}</span></span>
          </Tag>
          <Tag type="normal" size="small">
            <span>不符合预期：<span style={{ color: '#f69b00' }}>{stateCount.failed}</span></span>
          </Tag>
          <Tag type="normal" size="small">
            <span>异常：<span style={{ color: '#d93027' }}>{stateCount.error}</span></span>
          </Tag>
          <Tag type="normal" size="small">
            <span style={{ color: '#151515' }}>待运行：{stateCount.wait >= 0 ? stateCount.wait : 0}</span>
          </Tag>
        </TagGroup>
      </div>
    </div>
