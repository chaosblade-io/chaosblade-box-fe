...
export default function TaskInfo(props: TaskInfoProps): any {
  ...
  function handleShowExecuteInfo(item: IApp) {
    ...
    const title: any = <Tab className={styles.logTabs} triggerType="click" onChange={value => setExcuteTab(value)} extra={<Button type="primary" onClick={() => onRefresh()} style={{ margin: '-12px 32px 0 0', fontSize: '14px' }} text><Icon type="refresh"/> 刷新</Button>}>
      <Tab.Item key='run' title={'机器执行信息'}></Tab.Item>
    </Tab>;
  ...
  const renderDebugInfo = () => {
    ...
    if (!origin_request && !origin_response) {
      return (
        <>
          <div className={styles.infoTitle}>调试信息</div>
          {empty}
        </>
      );
    }
        ...
        <Loading visible={loadingResult} style={{ display: 'block' }}>
          <div className={styles.infoContainer}>
            { hostDetail &&
              <>
                ...
                <div className={styles.detailInfo}>
                  ...
                  <div><span>机器类型: </span>{hostDetail.type}</div>
                  {hostDetail.clusterName && <div><span>集群: </span>{hostDetail.clusterName}</div>}
                  {hostDetail.kubNamespace && <div><span>namepace: </span>{hostDetail.kubNamespace}</div>}
                  <div><span>开始时间：</span>{formatDate(startTime)}</div>
                  {endTime && <div><span>结束时间：</span>{formatDate(endTime)}</div>}
                </div>
              </>
            }
            ...
            {info.data && !_.isNil(info.data) &&
              <>
                <div className={styles.infoTitle}>信息</div>
                <div className={styles.infoContent}><pre>{formatData(info.data)}</pre></div>
              </>
            }
            {info.errorMessage &&
              <>
                <div className={styles.infoTitle}>错误</div>
                <div className={styles.infoContent}>原因：{info.errorMessage}</div>
              </>
            }
            {solution &&
              <>
                <div className={styles.infoTitle}>排查</div>
  ...
  function renderManyMachines(apps: IApp[]) {
    ...
    return (
      <div className={styles.ipsContent}>
        {apps.length > 12 &&
          <Input hasClear placeholder='模糊搜索...' className={styles.ipsSearch} onChange={value => setSearchIps(_.trim(value))}/>
        }
  ...
  function renderMachineInfo() {
    ...
    return <div className={styles.machine}>
      <TagGroup>
        <Tag type="normal" size="small">成功：<span className={styles.success}>{successNum || 0}</span></Tag>
        <Tag type="normal" size="small">失败：<span className={styles.faile}>{failNum || 0}</span></Tag>
        <Tag type="normal" size="small">跳过执行: <span style={{ color: 'grey' }}>{rejectNum || 0}</span></Tag>
        <Tag type="normal" size="small">待运行：{readyNum || 0}</Tag>
      </TagGroup>
      <div className={styles.machineDetail}>
        <div className={styles.detailTop}>
          <div className={styles.iconAndWord}>
            ...
            <div className={styles.title}>机器（<span>{apps && apps.length}</span>）</div>
          </div>
          <div className={styles.iconAndWord}>
            <Icon type="help" className={styles.tipsIcon}/>
            <div className={styles.tips}>点击机器可查看详情</div>
          </div>
        </div>
        {apps && renderManyMachines(apps)}
      </div>
      <div className={styles.userCheckState}>
        <div className={styles.checkStateLabel}>用户确认结果</div>
        {userCheckState === ExperimentConstants.EXPERIMENT_ACTIVITY_TASK_USER_CHECK_STATE_PASSED &&
          <div className={styles.checkPass}>继续执行</div>}
        {
          userCheckState === ExperimentConstants.EXPERIMENT_ACTIVITY_TASK_USER_CHECK_STATE_FAILED && <div className={styles.checkFailed}>终止演练</div>
        }
  ...
  function handleShowParameter() {
    ...
    if (!_.isEmpty(argumentsParams)) {
      ...
      setDialogTitle(`${miniAppName}节点执行参数`);
    } else {
      return;
    }
  }
  ...
  return <div className={styles.infoContent}>
    <Tab shape="capsule" size="small" defaultActiveKey={1}>
      <Tab.Item title="机器信息" key={1}>
        {!_.isEmpty(data) && renderMachineInfo()}
      </Tab.Item>
      {React.isValidElement(Plugin) && !_.isEmpty(data) ? <Tab.Item title="执行动态" key={2}>
        <div>
          {Plugin}
        </div>
      </Tab.Item> : null }
    ...
    <div className={styles.actionCon}>
      {!_.isEmpty(data) && <span className={classnames(styles.action, noParamsStyle)} onClick={handleShowParameter} title={!argumentsParams.length ? '该节点无参数配置' : ''}>参数</span>}
