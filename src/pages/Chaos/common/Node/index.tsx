        ...
        editable && isHead && (
          <div className={styles.headBox}>
            <Iconfont
              ...
              data-autolog={`text=${i18n.t('Add a walkthrough node')}`}
            />
            ...
            <div className={styles.parameterInfo}>
              {
                !isExpertise && editable && !state && (
                  <>
                    <span><Translation>Required parameters</Translation>: </span>
                    { (hasRequiredParameters || nodeType === RECOVER) && <span style={{ color: '#D93026' }}><Translation>Have</Translation></span> }
                    { (!hasRequiredParameters && nodeType !== RECOVER) && <span><Translation>None</Translation></span> }
                  </>
                )
              }
              {
                ...
                !isExpertise && !editable && !state && (nodeType !== NORMAL) && (
                  <>
                    <span><Translation>Required parameters</Translation>: </span>
                    { (hasRequiredParameters || nodeType === RECOVER) && <span style={{ color: '#D93026' }}><Translation>Have</Translation></span> }
                    { (!hasRequiredParameters && nodeType !== RECOVER) && <Translation>None</Translation> }
                  </>
                )
              }
              {
              ...
              }
              {
                !isExpertise && !editable && (nodeType === NORMAL) && !state && (
                  <>
                    <span><Translation>Involving machines</Translation>: </span>
                    {(hostPercent && hostPercent !== 0 && !hosts) ? <span style={{ color: '#0070cc' }}>{hostPercent}%</span> : <span></span>}
                    {hosts &&
                      <span style={{ color: '#0070cc' }}>{`${node.hosts} ${i18n.t('Number')}`}</span>
                    }
                  </>
                )
              }
              {
                userCheckState === ExperimentConstants.EXPERIMENT_ACTIVITY_TASK_USER_CHECK_STATE_WAITING && <span><Translation>To be pushed manually</Translation></span>
              }
              {
                runResult && runResult === NodeRunResult.SUCCESS && <span><Translation>Node executes successfully</Translation></span>
              }
              {
                runResult && _.indexOf(NodeRunResult.FAILED, runResult) >= 0 && <span><Translation>Node execution failed</Translation></span>
              }
              {
                runResult && runResult === NodeRunResult.REJECTED && <span><Translation>Node execution is skipped</Translation></span>
              }
              {
                state && state === 'RUNNING' && <span><Translation>Node is executing...</Translation></span>
              }
            </div>
          </div>
            ...
            <div className={styles.userAction}>
              {
                retryable && <span className={styles.contiueTryIcon} onClick={e => handleTryAgain(e)}>
                  <Button type='primary' loading={isRetry} text className={styles.action} disabled={!handleIsAdmin(permission as number, 4)}><Translation>Retry</Translation></Button>
                </span>
              }
              {
                userCheckState === ExperimentConstants.EXPERIMENT_ACTIVITY_TASK_USER_CHECK_STATE_WAITING
                  ...
                  <span className={styles.userCheck}>
                    <span className={styles.contiueTryIcon} onClick={e => handleIsContiue(e, true)}>
                      <Button type='primary' loading={isContinue} text className={styles.action} disabled={!handleIsAdmin(permission as number, 4)}><Translation>Continue</Translation></Button>
                    </span>
                    <span onClick={(e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => handleIsContiue(e, false)}>
                      <Button type='primary' loading={isStop} text className={styles.action} disabled={!handleIsAdmin(permission as number, 4)}><Translation>Termination</Translation></Button>
                    </span>
                  </span>
                  :
                  <span></span>
        ...
        }
        {
          ...
          editable && deletable && !state && (
            <Icon
              ...
              data-autolog={`text=${i18n.t('Delete walkthrough node')}`}
            />
          )
        }
        {
