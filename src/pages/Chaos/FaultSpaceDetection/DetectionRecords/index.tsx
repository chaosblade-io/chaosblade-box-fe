...
const DetectionRecords: FC = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const startDate = dateRange?.[0] ? formatDate(dateRange[0] as any) : undefined;
      const endDate = dateRange?.[1] ? formatDate(dateRange[1] as any) : undefined;
      const res = await probeProxy.listTaskExecutions({
      ...
      const mapped = items.map((it: any) => ({
        id: it.id,
        runId: String(it.id || it.executionId || it.runId || ''),
        status: String((it.status || 'DONE')).toUpperCase(),
        environment: it.namespace || '-',
        requestNum: Number(it.requestNum ?? it.request_num ?? 0),
        startTime: it.startedAt || it.started_at || it.createdAt || it.created_at || '',
        endTime: it.finishedAt || it.finished_at || it.updatedAt || it.updated_at || '',
        duration: Number(it.duration ?? it.durationSeconds ?? it.duration_seconds ?? 0),
      ...
      <div className={styles.tableContainer}>
        ...
        >
          ...
          <Table.Column
            ...
            width="12%"
            cell={(value: string) => (
              <span style={{ fontFamily: 'Monaco, Consolas, monospace', fontSize: 12 }}>{value}</span>
            )}
          />
        </Table>
