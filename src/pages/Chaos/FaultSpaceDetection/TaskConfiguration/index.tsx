...
import TargetSystemSection from './components/TargetSystemSection';
import APIParameterSection from './components/APIParameterSection';
import TraceVisualizationSection from './components/TraceVisualizationSection';
import SLOConfigurationSection from './components/SLOConfigurationSection';
import ExecutionConfigurationSection from './components/ExecutionConfigurationSection';
...
const TaskConfiguration: FC = () => {
  ...
  useEffect(() => {
    ...
    dispatch.pageHeader.setTitle(i18n.t('Task Configuration').toString());
    dispatch.pageHeader.setBreadCrumbItems(chaosDefaultBreadCrumb.concat([
      ...
      { key: 'task_configuration', value: i18n.t('Task Configuration').toString(), path: '/chaos/fault-space-detection/add' },
    ]));
  }, []);
  ...
  return (
    <div className={styles.container}>
      ...
      <Affix offsetTop={80} className={styles.sidebarAffix}>
  ...
  function calculateEstimatedDuration(): string {
    const { totalRequests, concurrency, requestTimeout } = formData.executionConfig;
    const { warmupSeconds, cooldownSeconds } = formData.executionConfig.warmupCooldown;

    const executionTime = Math.ceil(totalRequests / concurrency) * (requestTimeout / 1000);
    const totalTime = warmupSeconds + executionTime + cooldownSeconds;
