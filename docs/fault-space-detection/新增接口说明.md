1、获取系统信息
http://{starter-host}:7001/api/systems?page=1&size=20
GET
{
    "success": true,
    "data": {
...
2、获取API信息
http://{starter-host}:7001/api/systems/2/apis
GET
2 =》systemId
{
    "success": true,
...
3、获取拓扑
http://{starter-host}:7001/api/topologies/1/nodes
GET
1 => api_id
{
    "success": true,
...
4、获取故障类型
http://{starter-host}:7001/api/fault-types
{
    "success": true,
    "data": {
        "items": [
...
5、新增task
http://{starter-host}:7001/api/probe-tasks
POST
{
